...
`eval/` 下有一套 404 条的检索评测集和三个对比实验，完整记录见
[eval/README.md](eval/README.md)。结论：

| 方案 | Recall@1 | MRR | 延迟中位 |
|---|---|---|---|
| **纯向量**（线上采用） | **53.7%** | 63.2% | **103ms** |
| BM25 | 0.8% | 5.5% | 1ms |
| 混合（最佳权重 0.9/0.1） | 42.2% | 55.5% | 110ms |
| 向量 + Rerank | 56.0% | 64.4% | 2307ms |

- **BM25 混合有害**：语料 99.86% 英文、提问中文，词项匹配无从下手，
  任何权重都拖后腿
- **Rerank 不划算**：+2.3pp Recall@1 换 22 倍检索延迟；且百炼该端点没有
  rerank 模型，本地模型要引入 torch 让镜像涨到 3GB。仅作离线实验保留
- **阈值 0.45 经受住检验**：拒答准确率 100%，误拒率 0%

评测集是 LLM 从文章反向生成的合成数据，**绝对数值偏乐观**，适合横向对比、
不适合对外宣称准确率。局限在 eval/README.md 里写明了。
